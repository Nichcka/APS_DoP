...
def process_alignment_DNA(alignment_file, output_file_base):
    ...
    try:
        ...
        raise FileNotFoundError(f"File '{alignment_file}' not found.")
    except Exception as e:
        raise Exception(f"Error reading file '{alignment_file}': {e}")
...
def create_matrices(df, data_type):
    ...
    if data_type == 'dna':
        identity_matrix = pd.DataFrame(100, index=all_ids, columns=all_ids, dtype='float64')
        for _, row in df.iterrows():
            id1, id2 = row['Seq_1'], row['Seq_2']
            identity_matrix.loc[id1, id2] = identity_matrix.loc[id2, id1] = float(row['Per_Id'])
        return identity_matrix, None
    else:
        identity_matrix = pd.DataFrame(100, index=all_ids, columns=all_ids, dtype='float64')
        similarity_matrix = pd.DataFrame(100, index=all_ids, columns=all_ids, dtype='float64')
        for _, row in df.iterrows():
            id1, id2 = row['Seq_1'], row['Seq_2']
...
def parse_arguments():
    ...
    if args.mode == 'protein' and args.aminoacid is None:
...
def main():
    ...
    if args.mode == 'DNA':
        try:
            ...
            raise Exception(f"Error processing DNA alignment: {e}")
        ...
        try:
            ...
            raise Exception(f"Error processing protein alignment: {e}")
    ...
    try:
        ...
        raise Exception(f"Error during data processing or plotting: {e}")
